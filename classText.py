...
class Text (str):
	...
	def replace (self, oldWord, newWord=''):
		return Text (str.replace (self, oldWord, newWord))

	def strip (self, chars=""):
		if chars: return Text (str.strip (self, chars))
		else: return Text (str.strip (self))
	...
	def sliceWord (self, wordStart, wordEnd):
		res =""
		if wordStart in self and wordEnd in self:
			d= self.find (wordStart) + len (wordStart)
			f= self.find (wordEnd, d)
			if f>0 and f>d: res = self[d:f]
		return Text (res)
	...
	def test (self):
		print ('remplace\t', self.replace ('n','m'))
		print ('éfface\t', self.replace ('n'))
		print ('trouve\t', self.find ('sl'))
		print ('trouve\t', self.find ('sl', 20))
		print ('trouve\t', self.find ('sl', -20))
		print ('sliceWord\t', self.sliceWord ('emo', 'cqk'))
		print ('nettoie\t', self.clean())
		print ('majuscules\t', self.upperCase ('upper reset'))
