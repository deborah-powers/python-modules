...
class Mantis ():
	...
	def __str__ (self):
		message = '%s - %s: %sttttype: %stmodule: %s' % (self.numext, self.numint, self.message, self.type, self.module)
		return message
class MantisFile (Mantis, File):
	def __init__ (self, numext ='0', message =' ?', module = ' ?', numint ='0', type =' ?'):
		Mantis.__init__ (self, numext, message, module, numint, type)
		File.__init__ (self, 'b/mantis' + numext)
	def createFile (self):
		refFile.fromFile ()
		...
		commandesStr = 'log.debug ("________________________ requete ________________________");nlog.debug (obj.getA () +"t"+ obj.getB);'
		solutionStr = 'branche: %s mantis-%snreprise de donnée nécessaire: ?' % (self.module, self.numext)
		if self.type == 'ddt':
