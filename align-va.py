...
def buildMatrix (anyC, anyT):
	# fonctionne pour les list et les textes. les élements ont déjà été prépendus
	aliMatrix =[]
	aliPath =[]
	rangeA = range (1, len (anyC))
	rangeB = range (1, len (anyT))
	aliMatrix.append ([0])
	aliPath.append ([0])
	for b in rangeB:
...
#	listA.insert (0,'_')
	listA.prepend ('_')
	listB.prepend ('_')
	aliMatrix, aliPath = buildMatrix (listA, listB)
	# phase aller
	for a in rangeA:
		for b in rangeB:
			if listA[a] == listB[b]:
				aliMatrix[a][b] = scoreAli
				aliPath[a][b] =0
			else:
				aliMatrix[a][b] =0
				aliPath[a][b] = random.choice (scoreChoice)
	# phase retour
	listAnew =[]
	listBnew =[]
	return aliScore, listAnew, listBnew
...
def alignText (textA, textB):
	...
	textB ='_'+ textB
	aliScore =0
	aliMatrix, aliPath = buildMatrix (textA, textB)
	# phase aller
	for a in rangeA:
		for b in rangeB:
			aliScore =0
			if textA[a] == textB[b]: aliScore = scoreAli
			elif textA[a] + textB[b] in scoreMatrix.keys (): aliScore = scoreMatrix [textA[a] + textB[b] ]
			elif textB[b] + textA[a] in scoreMatrix.keys (): aliScore = scoreMatrix [textB[b] + textA[a] ]
			scoreTmp =[
				aliMatrix [a-1][b] + scoreGap,
				aliMatrix [a-1][b-1] + aliScore,
				aliMatrix [a][b-1] + scoreGap
			]
			aliMatrix[a][b] = max (scoreTmp)
			aliPath[a][b] = scoreTmp.index (aliMatrix[a][b]) -1
	# phase retour
	textAnew =""
	textBnew =""
	lenA = len (textA) -1
	...
	while lenA >0 or lenB >0:
		if aliPath [lenA][lenB] ==0:
			textAnew = textA [lenA] + textAnew
			textBnew = textB [lenB] + textBnew
			lenA -=1
			lenB -=1
			...
			textBnew = '_'+ textBnew
			lenA -=1
	aliScore = aliMatrix[-1][-1] / len (textAnew)
	return aliScore, textAnew, textBnew
