...
sourceStr = 'abc((de)f)g(h(ij)k)l(mn)opqrstu(vw)(xy)z'
source = list (sourceStr)

def findSister (source):
	sisA = source.index ('(')
	sisB = source.index (')', sisA)
	tot = source[sisA:sisB].count ('(')
	while tot >1:
		sisB = source.index (')', sisB +1)
		tot -=1
	destin =[]
	rangeTmp = range (sisA)
	for t in rangeTmp: destin.append (source[t])
	if '(' in source [sisA +1:sisB]:
		destinTmp = findSister (source [sisA +1:sisB])
		destin.append (destinTmp)
	else: destin.append (source [sisA +1:sisB])
	if '(' in source [sisB:]:
		destinTmp = findSister (source [sisB +1:])
		for item in destinTmp: destin.append (item)
	else:
		for item in source [sisB +1:]: destin.append (item)
	print (destin)
	return destin
